...
public record InitiateStopMiningCommand(RigId RigId, Guid UserId) : IUserableValidatableCommand<Unit>;


/// <summary>
/// Обработчик <see cref="InitiateStartMiningCommand"/>.
/// </summary>
public class InitiateStopMiningCommandHandler :
    BaseCommandHandler,
    IRequestHandler<InitiateStopMiningCommand, Result<Unit>>
{
    private readonly IRigGrainFactory _rigGrainFactory;

    public InitiateStopMiningCommandHandler(IMediator mediator,
                                            ILogger<InitiateStopMiningCommandHandler> logger,
    ...
    public async Task<Result<Unit>> Handle(InitiateStopMiningCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await rig.InitiateStopMining(request.UserId);

        if (!await HandleResult(result, cancellationToken))
        {
            return Result<Unit>.Error("Failed to initiate stop mining to rig");
        }
